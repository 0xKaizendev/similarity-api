...
Toast.Icon = function ToastIcon({ name, className, ...props }: ToastIconProps) {
  ...
  return (
    <div className='flex h-20 w-20 items-center justify-center rounded-full bg-slate-100'>
      <Icon className={classNameOptimization('h-10 w-10', className)} {...props} />
    </div>
  )
}
...
Toast.Title = function ToastTitle({ className, ...props }: ToastTitleProps) {
  return <p className={classNameOptimization('text-sm font-medium', className)} {...props} />
}
...
Toast.Description = function ToastDescription({
  ...
  return <p className={classNameOptimization('text-sm opacity-80', className)} {...props} />
}
